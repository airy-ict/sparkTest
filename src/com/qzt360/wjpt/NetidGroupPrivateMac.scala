...
/**
 * 找出私人专用的mac，上面产生的身份属于同一个人
 */
object NetidGroupPrivateMac {
  def main(args: Array[String]) {
    val conf = new SparkConf().setAppName("NetidGroupPrivateMac")
    val sc = new SparkContext(conf)
    //im分析
    //滤掉明显不合格的数据
    val imAll = sc.textFile("/user/wjpt/im").filter { x =>
      {
        var result = false
        val parts = x.split("\t")
    ...
    //找出私人专用mac，一个mac上产生的身份个数有限
    val imMacPub = imAll.map { x =>
      {
        val parts = x.split("\t")
        //mac netIdType netId
        parts(6) + "\t" + parts(7) + "\t" + parts(10)
      }
    }.distinct.map { x =>
      val parts = x.split("\t")
      (parts(0), 1)
    ...
    val emailMacPub = emailAll.map { x =>
      {
        val parts = x.split("\t")
        //mac netIdType netId
        parts(6) + "\t9\t" + parts(9).toLowerCase()
      }
    }.distinct.map { x =>
      val parts = x.split("\t")
      (parts(0), 1)
    ...
    val macPub = (imMacPub union emailMacPub).reduceByKey(_ + _)
    macPub.saveAsTextFile("/user/zhaogj/output/macPub")

    val macPubMap = macPub.collectAsMap
    val im = imAll.filter { x =>
      {
        var result = false
        val parts = x.split("\t")
        if ("None".equals("" + macPubMap.get(parts(6)))) {
          result = true
        }
        result
      }
    }
    val email = emailAll.filter { x =>
      {
        var result = false
        val parts = x.split("\t")
        if ("None".equals("" + macPubMap.get(parts(6)))) {
